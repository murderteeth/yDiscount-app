...
function AddressInput({
	...
	return (
		<>
			<div className={'flex h-10 w-full items-center bg-purple-800/40 p-2 transition-colors'}>
				<input
				...
				<label
					...
					<span className={status === 'invalid' || status === 'warning' ? 'tooltip' : 'pointer-events-none'}>
						<div className={'pointer-events-none relative h-4 w-4'}>
							<IconCheck
								className={`absolute h-4 w-4 text-purple-100 transition-opacity ${
									status === 'valid' ? 'opacity-100' : 'opacity-0'
								}`}
							/>
							<IconCircleCross
								className={`absolute h-4 w-4 text-orange-600 transition-opacity ${
									status === 'invalid' ? 'opacity-100' : 'opacity-0'
								}`}
							/>
							<div className={'absolute inset-0 flex items-center justify-center'}>
								<IconLoader
									className={`h-4 w-4 animate-spin text-purpl-400 transition-opacity ${
